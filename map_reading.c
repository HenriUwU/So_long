...
/*   Updated: 2023/02/02 16:43:25 by hsebille         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (data->mapping[j])
	{
		...
		while (data->mapping[j][i])
		{
			...
			if (data->mapping[j][i] == 'C')
				data->checker_collectible++;
			i++;
		}
		j++;
	}
	if (data->checker_player < 1 || data->checker_exit < 1
		|| data->checker_collectible < 1)
		return (0);
	return (1);
}
...
int	read_map(char **argv, t_data *data)
{
	...
	free(line);
	close(fd);
	if (is_rectangle(*data) == 0 || is_duplicate(*data) == 0
		|| is_map_logic(*data) == 0 || is_map_surrounded(*data) == 0)
		return (-1);
	return (0);
}
